...
{
	return CGameObject::LoadImage(m_Images,IDB_ME,RGB(0,0,0), PLANE_WIDTH, PLANE_HEIGHT,5);
}
BOOL CMyPlane::Draw(CDC* pDC,BOOL bPause)
{
	...
	if(!bPause)
	{
		m_ptPos.x = m_ptPos.x + m_nHorMotion*PLANE_SPEED;
		m_ptPos.y = m_ptPos.y - m_nVerMotion*PLANE_SPEED;
	}
...
BOOL CMyPlane::Draw(CDC* pDC, BOOL bPause, int level)
{
	m_nWait++;
	if (m_nWait>3)
		m_nWait = 0;
